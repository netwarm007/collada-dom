...
void Thumbnail::SetDocument(const char *document)
{
	...
	if(strstr(document,MEDIA_MOUNT_PATH[i])!=nullptr)
	{
		m_media_type = i; return;
	}
}
...
void BrowserUI::ParseDir(const char *path)
{
	...
	err = closedir(dir); API_ERROR(err);
}
...
void BrowserUI::Init()
{
	...
	else m_TextureMap[DEFAULT_IMAGE] = ret;
}
...
bool BrowserUI::AddThumbnail(const char *document,const char *image)
{
	...
	if(image)
	{
		...
		{ //image file not exist, use default texture
			tn->SetImage(DEFAULT_IMAGE);
			tn->SetTexture(m_TextureMap[DEFAULT_IMAGE]);
		}
	}
...
GLuint LoadTextureRAW(const char *filename, int wrap)
{
	...
	delete[] data; delete[] raw; return texture;
}
