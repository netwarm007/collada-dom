...
//this file name can be one of
//domSource.h or domSource_type.h, so
//including domCOLLADA gets the correct file
	...
	daeElement* findSidTopDown(daeElement* container, const string& sid, const string& profile) {
		...
		for (size_t i = 0; i < elts.size(); i++) {
			...
			if (!profile.empty()) {
				...
				if (strcmp(elt->getElementName(), "technique_common") == 0)
					continue;
				//if (strcmp(elt->getElementName(), COLLADA_ELEMENT_TECHNIQUE) == 0  &&
				if (strcmp(elt->getElementName(), "technique") == 0  &&
